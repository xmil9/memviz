...
static void workerMain(Shared* S)
{
   ...
   while (S->running.load())
   {
      ...
      uint64_t budget;
      {
         ...
         attach = S->reqAttach;
         detach = S->reqDetach;
         once = S->reqRefreshOnce;
         autoR = S->autoRefresh;
         mode = S->mode;
         width = S->gridWidth;
         rms = S->refreshMs;
         budget = S->contentBudget;
      ...
      if (attach)
      {
         ...
         if (h)
         {
            ...
            S->attachedName = name;
            S->status =
               "Attached to " + narrow(name) + " (pid " + std::to_string(pid) + ")";
         }
         else
         {
      ...
      if (doScan)
      {
         ...
         if (mode == model::ColorMode::Content)
         {
            ...
            if (prev)
            {
               model::markChanges(*prev, *grid);
            }
         }
         char buf[256];
         std::snprintf(buf, sizeof(buf),
                       "%zu regions | committed %.1f MB | reserved %.1f MB",
                       grid->stats.regionCount, grid->stats.committedBytes / 1048576.0,
...
static size_t findRegion(const model::Grid& g, uintptr_t addr)
{
   ...
   size_t idx = lo - 1;
   if (addr >= R[idx].base && addr < R[idx].base + R[idx].size)
   {
      return idx;
   }
   return SIZE_MAX;
...
{
   g_procList = memscan::listProcesses();
}
                               ...
                               gfx::Texture& tex, float zoom)
{
   ...
   int dstW = 0, dstH = 0;
   model::upscalePixels(grid->pixels, grid->width, grid->height, g_texCellPx,
                        g_upscaledPixels, dstW, dstH);
...
static void drawProcesses(Shared& S)
{
   ...
   ImGui::InputTextWithHint("##filter", "filter by name or pid", g_filter,
                            sizeof(g_filter));

   ImGui::BeginChild("proclist", ImVec2(0, 0), ImGuiChildFlags_Border);
   char lower[128];
   for (int i = 0; g_filter[i] && i < 127; ++i)
      lower[i] = (char)tolower(g_filter[i]), lower[i + 1] = 0;
   if (!g_filter[0])
   {
      lower[0] = 0;
   }
   ...
   for (const auto& p : g_procList)
   {
      ...
      if (lower[0])
      {
         ...
         if (ln.find(lower) == std::string::npos &&
             pidstr.find(lower) == std::string::npos)
         {
            continue;
         }
      }
      char label[300];
      std::snprintf(label, sizeof(label), "%-6lu  %s", (unsigned long)p.pid,
                    name.c_str());
...
static void drawCanvas(Shared& S, const std::shared_ptr<const model::Grid>& grid,
                       gfx::Texture& tex)
{
...
static void drawDetails(Shared& S, const std::shared_ptr<const model::Grid>& grid)
{
   ...
   size_t got = 0;
   {
      ...
      if (S.handle)
      {
         got = memscan::readMemory(S.handle, g_selAddr, data.data(), previewBytes);
      }
   }
   if (got == 0)
   {
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, int)
{
   ...
   WNDCLASSEXW wc = {sizeof(wc), CS_CLASSDC, WndProc,      0,
                     0,          hInst,      nullptr,      nullptr,
                     nullptr,    nullptr,    L"MemVizWnd", nullptr};
   RegisterClassExW(&wc);
   HWND hwnd =
      CreateWindowW(wc.lpszClassName, L"MemViz - Memory Visualizer", WS_OVERLAPPEDWINDOW,
                    80, 60, 1360, 860, nullptr, nullptr, wc.hInstance, nullptr);
