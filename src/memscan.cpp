...
static RegionState toState(DWORD s) {
    if (s == MEM_COMMIT) return RegionState::Committed;
    if (s == MEM_RESERVE) return RegionState::Reserved;
...
std::vector<Region> enumRegions(HANDLE h, uintptr_t lo, uintptr_t hi) {
    ...
    while (addr < hi) {
        ...
        if (next <= addr) break;  // guard against wrap / zero size
        addr = next;
    }
    return out;
}
...
const char* stateStr(RegionState s) {
    switch (s) {
        case RegionState::Committed: return "Committed";
