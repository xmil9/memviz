...
static uint32_t metadataColor(const memscan::Region& r) {
    ...
    if (r.state == RegionState::Reserved) return col::Reserved;
    if (r.state != RegionState::Committed) return col::Reserved;

    if (r.protect & PAGE_GUARD) return col::Guard;
    if ((r.protect & 0xFF) == PAGE_NOACCESS || r.protect == 0) return col::NoAccess;

    const bool ex = hasExec(r.protect);
    const bool wr = hasWrite(r.protect);
    switch (r.type) {
        case MEM_IMAGE:
            if (ex) return col::ImageExec;
            return wr ? col::ImageRW : col::ImageRO;
        case MEM_MAPPED:
            return col::Mapped;
        default:  // MEM_PRIVATE
            if (ex) return col::PrivateExec;
...
std::vector<LegendItem> contentLegend() {
    return {
        {"Low entropy",    rgba(40, 90, 200)},
        {"Medium entropy", rgba(70, 190, 90)},
        {"High entropy",   rgba(220, 70, 60)},
        {"Zeroed",         col::Zeroed},
        {"Unreadable",     col::Unreadable},
        {"Not sampled",    col::Unsampled},
        {"Reserved",       col::Reserved},
        {"Changed",        rgba(255, 244, 120)},
    };
}
...
std::shared_ptr<Grid> buildGrid(const std::vector<memscan::Region>& all, int width) {
    ...
    for (const auto& r : all) {
        if (r.state == memscan::RegionState::Free) continue;
        g->regions.push_back(r);
        if (r.state == memscan::RegionState::Committed) g->stats.committedBytes += r.size;
        else g->stats.reservedBytes += r.size;
    }
...
void fillContent(HANDLE h, Grid& g, uint64_t byteBudget) {
    ...
    for (size_t ri = 0; ri < g.regions.size(); ++ri) {
        ...
        if (r.state != memscan::RegionState::Committed) {
            for (size_t i = start; i < end; ++i) g.pixels[i] = col::Reserved;
            continue;
        }
...
void markChanges(const Grid& prev, Grid& cur) {
    if (prev.totalCells != cur.totalCells) return;
    if (prev.regions.size() != cur.regions.size()) return;
    if (prev.checksums.size() != cur.checksums.size()) return;
